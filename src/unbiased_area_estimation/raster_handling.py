...
def merge_classes(raster_path, out_raster_path, class_merge_dict):
    ...
    output_dataset = driver.CreateCopy(
        out_raster_path, base_ds, options=["COMPRESS=DEFLATE"]
    )
...
def rasterize(
    vector_path, raster_outpath, resolution, dtype="Byte", nodata=0, compress="DEFLATE"
):
...
def mask_raster_by_vector(
    ...
    rasterized_vector_path = rasterize(
        vector_path, tmp_masked_raster_path, resolution, dtype, src_nodata, compress
    )
...
def read_raster_classes(file_path):
    ...
    with rio.open(file_path) as dataset:
        for ji, window in dataset.block_windows(1):
            ...
            unique_classes.update(np.unique(band))
    print("Classes read.")
